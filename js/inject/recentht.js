...
var SK_apps_recentht={
	...
	initUI:function(){
		...
		dom.innerHTML=
			'<div class="sk_box_head" style="">'
				+'<span class="sk_box_title">'+sde.apps.i18n("recentht")+'</span>'
		...
		for(var i=0;i<this.cons.ht.length;i++){
			var list_li=sde.apps.domCreate("li",{setName:["className"],setValue:["sk_recentht_li"]},"<span class='li_0'>"+this.cons.ht[i].title+"</span><span class='li_1'>"+this.cons.ht[i].url+"</span>","",{setName:["id"],setValue:[i]},"");
	...
	handleEvent:function(e){
		switch(e.type){
			case"click":
				if(e.target.classList.contains("sk_recentht_li")||e.target.parentNode.classList.contains("sk_recentht_li")){
					var theDom=e.target.classList.contains("sk_recentht_li")?e.target:e.target.parentNode;
					chrome.runtime.sendMessage({type:"apps_action",apptype:"recentht",link:SK_apps_recentht.cons.ht[theDom.dataset.id].url},function(response){})
					if(SK_apps_recentht.cons.config.n_closebox){
						sde.apps.boxClose(e)
					}
				}
				break;
...
chrome.runtime.sendMessage({type:"apps_getvalue",apptype:"recentht"},function(response){
	SK_apps_recentht.cons.config=response.config;
	SK_apps_recentht.cons.ht=response.value.ht;
	SK_apps_recentht.initUI()
})